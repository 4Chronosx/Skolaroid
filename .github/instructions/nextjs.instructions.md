...
### Lesson: useEffect usage

**Always use `useEffect` only in Client Components (with `'use client'` at the top). Server Components cannot use React hooks like `useEffect`.**
...
Never use `useEffect` in Server Components (no `'use client'` directive).
